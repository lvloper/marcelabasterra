...
<x-block class="border-y border-gray-2 bg-gray-3 py-16 sm:py-20 lg:py-24">
    ...
    <div class="mx-auto max-w-[1440px] px-5 pb-16 pt-12 sm:px-8 lg:px-12 xl:px-16">
        ...
        <div class="grid gap-12 lg:grid-cols-12 lg:gap-8 xl:gap-12">
            ...
            <a href="{{ $libro?->url ?? route('publications.books') }}" class="group flex flex-col justify-center border-t-2 border-primary pt-6 text-left lg:col-span-4 order-1 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-accent">
                @if ($title ?? null)
                ...
                @if ($bookTitle)
                    <h3 class="mt-2 max-w-[18ch] font-sans text-[clamp(1.5rem,2.5vw,2.25rem)] font-normal leading-[.95] tracking-[-.02em] text-primary transition-colors duration-300 group-hover:text-primary/70">{{ $bookTitle }}</h3>
                @endif
                ...
                @if ($displayDate)
                    <p class="mt-8 font-source text-[clamp(2rem,3.5vw,3.5rem)] leading-none text-primary">{{ $displayDate }}</p>
                @endif

                @if ($bookPublisher)
                    <p class="mt-3 font-source text-sm text-gray">{{ $bookPublisher }}</p>
                @endif

                <span class="mt-8 inline-flex min-h-11 w-fit items-center border-b border-primary font-body text-base font-semibold text-primary transition-colors duration-300 group-hover:border-accent" aria-hidden="true">
                    {{ $ctaLabel }} <span class="ml-2 transition-transform duration-300 group-hover:translate-x-1" aria-hidden="true">→</span>
                </span>
            </a>

            <div class="flex items-center justify-center border-t-2 border-primary pt-6 lg:col-span-4 order-3 lg:order-2">
            ...
            <div class="flex flex-col border-t-2 border-primary pt-6 text-left lg:col-span-4 order-2 lg:order-3">
                @if ($otherBooks->isNotEmpty())
                    <h3 class="border-l-2 border-accent pl-3 font-source text-base font-semibold text-primary">Otros libros publicados</h3>
                    <ul class="mt-4">
                        @foreach ($otherBooks as $other)
                            <li>
                                <a href="{{ $other->url }}" class="group flex items-start gap-4 border-b border-gray-2 py-4 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-accent">
                                    @if ($other->portada)
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <p class="font-source text-[1.35rem] leading-none text-primary/70">{{ $other->fecha_publicacion?->year ?? 'Sin año' }}</p>
                                        <p class="mt-2 font-sans text-base leading-tight text-primary transition-colors group-hover:text-primary/70">{{ $other->title }}</p>
                                    </div>
