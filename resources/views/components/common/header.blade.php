...
hidden lg:block
fixed top-0 z-50 w-full bg-transparent" x-data="{ 
    ...
    :class="{ 'sticky-active bg-white shadow-md': scrolled || menuBarMenu }">


    {{-- @persist('header') --}}
    <div class="flex justify-between px-6 container mx-auto items-center min-h-[80px] transition-all duration-300"
        :class="{ 'min-h-[46px]': scrolled }">
        <div class="py-2">
            <a href="/" wire:navigate.hover class="block">
                <img src="{{ asset('img/Logo-blanco.svg') }}"
                     alt="{{ config_text('site-name', 'CMS Base') }}"
                     class="h-10 transition-all duration-300"
                     :class="{ 'h-8': scrolled }"
                     x-show="!scrolled">
                <img src="{{ asset('img/Logo-azul.svg') }}"
                     alt="{{ config_text('site-name', 'CMS Base') }}"
                     class="h-10 transition-all duration-300"
                     :class="{ 'h-8': scrolled }"
                     x-show="scrolled" style="display:none">
            </a>
        </div>
                ...
                <div x-data="{
                    ...
                    <div class="relative top-0 left-0 z-40 w-auto h-10 transition duration-200 ease-out">
                        <div class="w-full h-full">
                            <div class="flex gap-6 w-full h-full select-none"
                                ...
                                <div class="relative h-full cursor-default" @mouseleave="closeMenu()">
                                    ...
                                    <div class="h-full"
                                        ...
                                        <x-link allowWireNavitage
                                            class="nav-link flex items-center h-full gap-2 font-medium transition-all duration-200 text-base {{ $route && !$item['route']['anchor'] && $route->isActiveParent() ? 'nav-link-active' : '' }}"
                                            :attrs="$item['route']">
                                            {{ $item['label'] }}
                                    ...
                                    <div x-show="menuBarMenu == 'menu{{$parentKey}}'" x-cloak @mouseover="cancelClose()"
                                        ...
                                        class="absolute left-0 z-50 p-4 mt-2 space-y-4 bg-white border rounded-lg shadow-lg w-max text-neutral-900">
                                    ...
                                    @else
                                    <x-link allowWireNavitage
                                        class="nav-link flex items-center h-full transition-all duration-200 text-base {{ $route && $route->isActive() ? 'nav-link-active font-medium' : '' }}"
                                        :attrs="$item['route']">
                                        {{ $item['label'] }}
                                    </x-link>
                                    @endif
    ...
    <style>
        #mainHeader:not(.sticky-active) .nav-link { color: white; }
        #mainHeader:not(.sticky-active) .nav-link:hover { color: rgba(255,255,255,0.7); }
        #mainHeader.sticky-active .nav-link { color: #171717; }
        #mainHeader.sticky-active .nav-link:hover { color: var(--color-primary); }
        #mainHeader.sticky-active .nav-link-active { color: var(--color-primary); }
        #mainHeader:not(.sticky-active) .nav-link-active { color: white; }
    </style>
</header>
